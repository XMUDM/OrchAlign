...
#     def forward(self, batch, encoder, model, decoder, _state):
#         seqs, bio_mask, peak_mask, rna_feat, signals, mask_regions, y_expr, eid = batch
#         pred_expr, aux_infor, mask,seq_input_embeds,signal_input_embeds = model(seqs=seqs, signals=signals, mask_regions=mask_regions, bio_mask=bio_mask,
#                                            peak_mask=peak_mask, rna_feat=rna_feat)
#         return pred_expr, y_expr, {},seq_input_embeds,signal_input_embeds
class ExtractRationale(BaseTask):
    def forward(self, batch, encoder, model, decoder, _state):
        seqs, signals, mask_regions, y_expr, eid, strand = batch
    
        out = encoder((seqs,signals))
   
        pred_expr, seq_input_embeds,signal_input_embeds = model(seqs=out[0][0], signals=out[0][1], strand=strand, mask_regions=mask_regions)
