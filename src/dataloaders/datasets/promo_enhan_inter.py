...
def get_padded_seq(chr_length, data_source, chrm, tss, each_total_lens, genome='hg38', data_type='seq', df_blacklist=None):
    ...
    start = max(0, tss - each_total_lens) #18425607-10000=18415607
    ...
    if chrm=='chrX' and tss==101386190:
        print('tss#',chrm,start,end,left_padding,right_padding)
...
class ExperInteractDataset(Dataset):
    def __init__(
        ...
        with open(os.path.join(self.data_folder, 'chr_length.json'), 'r') as file:
            self.chr_lengths = json.load(file)
        #CAGE data from the FANTOM5 project (Andersson et al., 2014) (K562: CNhs11250; GM12878: CNhs12333).
        # 预测目标
    ...
    def init_worker(self):
        ...
        #2,2,1
            ...
            #self.dnase = pyBigWig.open(os.path.join(self.data_folder, 'promo_enhan_inter', 'ATAC-seq', self.cell_type, 'ENCFF180ZAY.bigWig'))
        ...
        print('H5#,',len(self.data_h5['ensid']),self.data_h5['ensid']) # 18377 <HDF5 dataset "ensid": shape (18377,), type "|O">
        # MACS peak regions
    ...
    def init_datasets(self):
        if self.cell_type == 'K562':
            ...
            self.promoter_df = pd.read_csv(os.path.join(self.data_folder, 'promo_enhan_inter',
                                                        ...
                                                        'GeneList.txt'),
                                           sep='\t', index_col='symbol')
            print('promoter_df#,\n',self.promoter_df.loc['ENSG00000000003'])
            print('promoter_df_names#,',self.promoter_df.columns)
            #The promoter activity feature is defined as the square root of the product of DNase-seq and H3K27ac signal values.
            self.promoter_df['PromoterActivity'] = np.sqrt(self.promoter_df['H3K27ac.RPM.TSS1Kb']*self.promoter_df['DHS.RPM.TSS1Kb'])
            # enhancer positions & abc features  https://www.encodeproject.org/files/ENCFF969CYX/
            self.enhancer_gene = pd.read_csv(os.path.join(self.data_folder, 'promo_enhan_inter',
                                                          'GM12878_DNase_ENCFF020WZB_hic_4DNFI1UEG1HD_1MB_ABC_nominated',
                                                          'Gene_enhancer_links',
                                                          'EnhancerPredictionsAllPutative.txt.gz'), sep='\t')
            ...
            # enhancer的长度是500
            print('enhancer_gene#,',self.enhancer_gene.head(1)) #[326005 rows x 24 columns]
            ...
            self.ensid_names = pd.read_csv(os.path.join(self.data_folder, 'promo_enhan_inter',
                ...
                ['ENSID', 'Gene name']]
        print('self.split_df#,',self.split_df) #[18377 rows x 16 columns]
        if self.cell_type == 'H1':
            ...
            #print('enhancer_gene##,',self.enhancer_gene) #[326005 rows x 25 columns]
            # promoter_df 中的name对应ENSID
            gene_tss = self.promoter_df[['name', 'chr', 'tss', 'strand']].merge(self.split_df[['ENSID', 'Gene name']],
                                                                                left_on='name', right_on='ENSID').drop(
                columns='name')
            #print('gene_tss##,',gene_tss)   #[18377 rows x 5 columns]                                                                 
    ...
    def __getitem__(self, idx):
        sample_ensid = self.data_h5['ensid'][idx].decode()  # e.g. 'ENSG00000000003'
        # ENSG00000259431
        said=self.promoter_df.iloc[idx].name
        # assert sample_ensid==said
        print('isSame#,',sample_ensid==said)
        # 来自epiformer  mask没用到，这行代码可去掉；说明self.enhancer_gene也可以不用
        ...
        # mask selected by biology bio_masks
        true_select_mask, epinformer_select_mask = enh_masks[0], enh_masks[1]
        #print('mask#,',true_select_mask,epinformer_select_mask)
        ...
        if self.cell_type == 'H1':
            ...
            chrm, tss = self.promoter_df.loc[sample_ensid][['chr', 'tss']]
            #print('chrmtss#,',chrm,tss) # chr16 668131 ,TSS 是 start
        # promo_str = self.genome[chrm][tss-1000:tss+1000].seq
        # promo_str_h5 = one_hot_to_dna_str(seq_code[0])
        # assert promo_str == promo_str_h5
        each_total_lens = self.seq_range // 2  # 2k /2 20000
        # if sample_ensid=='ENSG00000196924':
        #     print(chrm, tss)
        seq_code_seqs = get_padded_seq(self.chr_lengths, self.genome, chrm, tss, each_total_lens, data_type='seq')
        # seq_code_seqs = self.genome[chrm][tss - each_total_lens: tss + each_total_lens].seq
        assert len(seq_code_seqs) == each_total_lens * 2
        if self.tokenizer_name == 'char':
            ...
            seq_code = dna_str_to_one_hot(seq_code_seqs).reshape(self.seq_range, 4) # one-hot 编码
        ...
        #bio_masks, peak_mask false 
        return seq_code_tensor, bio_masks, peak_mask, rnaFeat_tensor, signal_tensor, mask_regions, expr_tensor, sample_ensid
        #return seq_code_tensor,  rnaFeat_tensor, signal_tensor, mask_regions, expr_tensor, sample_ensid
        
        

class ExperAliDataset(Dataset):
    def __init__(
            self,
            data_folder,
            split_df=None,
    ...
    def __getitem__(self, idx):
        ...
        if self.cell_type == 'H1':
          ...
          # 正链 or 负链
        strand_symbol= self.promoter_df.loc[sample_ensid]['strand']
        ...
        strand_map = {'+': 0, '-': 1}
        strand = torch.nn.functional.one_hot(torch.tensor(strand_map[strand_symbol]), num_classes=2).float()
        ...
        # promo_str_h5 = one_hot_to_dna_str(seq_code[0])
        # assert promo_str == promo_str_h5
        each_total_lens = self.seq_range // 2  # 2k /2
        ...
        # assert len(hic) == each_total_lens * 2
        #concat 多模态信号
        signal_tensor = torch.concat((h3k27ac.unsqueeze(-1), dnase.unsqueeze(-1)), dim=-1)
        #signal_tensor = h3k27ac.unsqueeze(-1)
        #signal_tensor = dnase.unsqueeze(-1)
        #signal_tensor = torch.concat((h3k27ac.unsqueeze(-1), dnase.unsqueeze(-1), hic.unsqueeze(-1)), dim=-1)
        # blacklist & include list
        blacklist = get_padded_seq(self.chr_lengths, self.blacklist, chrm, tss, each_total_lens, data_type='mask')
        includelist = get_padded_seq(self.chr_lengths, self.includelist, chrm, tss, each_total_lens, data_type='mask')
        ...
        assert not torch.isnan(expr_tensor).any()
        #bio_masks, peak_mask false 
        return seq_code_tensor, signal_tensor, mask_regions, expr_tensor, sample_ensid, strand
        #return seq_code_tensor,  rnaFeat_tensor, signal_tensor, mask_regions, expr_tensor, sample_ensid