...
from src.models.sequence.GeneExpformer import sinkhorn_log_domain, squared_euclid
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        print('encoder_cfg,',encoder_cfg) #['genome', None]
        ...
        print('self.task.encoder#,',self.task.encoder,self.task.decoder)
    ...
    def reset_torch_metrics(self):
        ...
        #
        self.all_beforeOT_test=[]
        self.all_beforeOT_val=[]
        
        #12-24
        self.all_beforeOT_loss_test=[]
        self.all_beforeOT_loss_val=[]
        self.all_afterOT_loss_test=[]
        self.all_afterOT_loss_val=[]
        self.all_spec_loss_test=[]
        self.all_spec_loss_val=[]
        self.all_speckl_loss_test=[]
        self.all_speckl_loss_val=[]
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
       ...
       # x, y, w = self.forward(batch) # pred_expr, y_expr, aux_infor
        ...
        metrics['oth_loss'] = orth_coef*sig
        
        metrics['kl_loss'] = kl_coef*seq
        if 'mask' in w: 
...
def create_trainer(config, **kwargs):
    ...
    # ----------------------
    # resume_ckpt = getattr(config.train, "resume_from_checkpoint", None)
    # if resume_ckpt:
    #     log.info(f"Resuming training from checkpoint: {resume_ckpt}")
    #     kwargs['resume_from_checkpoint'] = resume_ckpt
    ...
    #trainer = hydra.utils.instantiate(config.trainer, callbacks=callbacks, logger=logger,**kwargs)
    return trainer
...
def train(config):
    ...
    # test_loaders = model.dataset.test_dataloader(**model.hparams.loader)
    # print('test_loader#,',len(test_loaders))
    # for i, batch in enumerate(test_loaders):
    #     print(f"Batch {i}:",batch)
        # for k, v in batch.items():
        #     print(f"  {k}: {v.shape if hasattr(v, 'shape') else type(v)}")

    print(f"Resuming training: global_step={trainer.global_step}") 
    ...
    if config.train.test: #test: True
...
def move_to_cuda(batch):
    if isinstance(batch, torch.Tensor):
        ...
        return [move_to_cuda(v) for v in batch]
    else:
        return batch  
...
def one_fold_training(fold, ori_wandb_name, ori_wandb_id, config_best_model_path, config):
    ...
    fold_config.dataset.k_fold = fold #11
...
def main(config: OmegaConf):
    ...
    if config.train.only_test: # false
        ...
        elif config.train.num_fold_CV: #12
            all_test_result = []
